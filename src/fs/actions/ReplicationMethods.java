    ...
    public void fileReplication(NetNodeLocation netNodeLoc, ReplicationWrapper repWr, NetNode nNode) {
        ...
        try {
            for (Map.Entry<Integer, NetNodeLocation> entry : nNode.getHashMap().entrySet()) {
                if (!nNode.getFileNodeList().get(repWr.getUFID()).getLocations().contains(entry.getValue())) {
        ...
        public void updateWritePermissionMap (String fileID, Collection < NetNodeLocation > nodeSet, ListFileWrapper
        listFileWrapper){
            Registry registry;
            try {
